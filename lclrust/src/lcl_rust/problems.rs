...
impl Problem for TSP{
    ...
    fn delta(&mut self, indices:(usize, usize)) -> isize {
        ...
        return if self.swap {
            ...
            initialscore += self.distance_matrix
                [self.solution[indices.0 - 1]]
                [self.solution[indices.0]];
            initialscore += self.distance_matrix
                [self.solution[indices.0]]
                [self.solution[indices.0 + 1]];
            initialscore += self.distance_matrix
                [self.solution[indices.1 - 1]]
                [self.solution[indices.1]];
            initialscore += self.distance_matrix
                [self.solution[indices.1]]
                [self.solution[indexsafe]];

            self.solution.swap(indices.0, indices.1);

            nextscore += self.distance_matrix
                [self.solution[indices.0 - 1]]
                [self.solution[indices.0]];
            nextscore += self.distance_matrix
                [self.solution[indices.0]]
    ...
    fn eval(&self) -> usize {
        ...
        for i in 1..self.size {
            score+=self.distance_matrix
                [self.solution[i-1]]
                [self.solution[i]];
        }
        score+=self.distance_matrix
            [self.solution[self.size-1]]
            [self.solution[0]];
